...
const shareOnWhatsApp = (news, router) => {
  const url = encodeURIComponent(`${router.basePath}/noticia/${news.categoryKey}/${news.id}`);
  const title = encodeURIComponent(news.title);
  window.open(`https://wa.me/?text=${title}%20${url}`, '_blank');
};

const shareOnFacebook = (news, router) => {
  const url = encodeURIComponent(`${router.basePath}/noticia/${news.categoryKey}/${news.id}`);
  window.open(`https://www.facebook.com/sharer/sharer.php?u=${url}`, '_blank', 'width=600,height=400');
};

const shareOnLinkedIn = (news, router) => {
  const url = encodeURIComponent(`${router.basePath}/noticia/${news.categoryKey}/${news.id}`);
  const title = encodeURIComponent(news.title);
  window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${url}&title=${title}`, '_blank', 'width=600,height=400');
};

const renderNewsCard = ({ news, router }) => {
  return (
...
export default function Home({ newsData, loading, error, currentDate }) {
  ...
  const latestNews = allNews.slice(4, 14); // ✅ Solo 10 últimas noticias
  const paginatedNews = allNews.slice(14); // Resto para paginación

  // ✅ Paginación: 10 por página
  const page = parseInt(router.query.page) || 1;
  const startIndex = (page - 1) * 10;
  const paginatedSlice = paginatedNews.slice(startIndex, startIndex + 10);
  const totalPages = Math.ceil(paginatedNews.length / 10);
      ...
      <Layout currentDate={currentDate}>
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          <div className="lg:col-span-4">
                ...
                {featuredNews.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {featuredNews.map(news => (
                      <div key={news.id} className="block bg-gradient-to-br from-blue-50 to-white rounded-xl shadow-lg border border-blue-200 overflow-hidden">
              ...
              <div className="bg-white rounded-2xl shadow-xl border border-blue-100 overflow-hidden mt-8">
                ...
                <div className="p-6">
                  <div className="space-y-6">
                    {latestNews.map(news => renderNewsCard({ news, router }))}
                  </div>
                </div>
              </div>
            )}

            {/* ✅ PAGINACIÓN: resto de noticias */}
            {paginatedSlice.length > 0 && (
              <div className="bg-white rounded-2xl shadow-xl border border-blue-100 overflow-hidden mt-8">
                <div className="bg-gradient-to-r from-blue-900 to-blue-700 p-6">
                  <h2 className="text-2xl font-bold text-white">Más Noticias</h2>
                  <div className="w-24 h-1 bg-red-500 mt-2"></div>
                </div>
                <div className="p-6">
                  <div className="space-y-6">
                    {paginatedSlice.map(news => renderNewsCard({ news, router }))}
                  </div>
                    ...
                    <div className="bg-gray-50 px-6 py-4 flex justify-center items-center space-x-2 mt-6">
                      ...
                      {[...Array(totalPages)].map((_, i) => (
                        <button 
                          ...
                          className={`px-4 py-2 rounded-lg ${page === i + 1 ? 'bg-blue-600 text-white' : 'bg-white text-blue-600 hover:bg-blue-100'} transition-colors`}
                        >
                          {i + 1}
                        </button>
                      ))}
            ...
            {/* SECCIONES POR CATEGORÍA (solo si hay noticias extra) */}
            {Object.entries(newsData).map(([categoryKey, newsList]) => {
              ...
              return (
                <div key={categoryKey} className="bg-white rounded-2xl shadow-xl border border-blue-100 overflow-hidden mt-8">
                  ...
                  <div className="p-6">
                    <div className="space-y-6">
                      {newsList.slice(0, 5).map(news => renderNewsCard({ news, router }))}
                    </div>
                  </div>
                </div>
              );
